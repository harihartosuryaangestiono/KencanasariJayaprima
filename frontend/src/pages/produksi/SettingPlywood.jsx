...
export default function SettingPlywood() {
  ...
  const settingMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Setting plywood berhasil dicatat');
      resetForm();
    },
    onError: (error) => {
      toast.error(error.response?.data?.error || 'Gagal menyimpan setting plywood');
    },
  });
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.shortcore_qty || !formData.face_qty || !formData.back_qty || !formData.lem_qty) {
      toast.error('Mohon lengkapi semua field yang wajib diisi');
      return;
    }
    
    const data = {
      tipe_plywood: formData.tipe_plywood,
      ...
      // hasil_ok dan hasil_reject tidak dikirim karena tidak ada di tabel setting_plywood
      // Kolom tersebut hanya ada di hotpress_log
    };
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-sm text-blue-800">
                    <strong>Catatan:</strong> Hasil OK dan Reject akan dicatat saat proses Hotpress, bukan di setting plywood.
                  </p>
                </div>
              </div>
            </div>
