...
export default function CoreBuilder() {
  ...
  const coreBuilderMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Proses core builder berhasil dicatat');
      resetForm();
    },
    onError: (error) => {
      toast.error(error.response?.data?.error || 'Gagal memproses core builder');
    },
  });
  ...
  const handleSubmit = (e) => {
    ...
    const hasil4x4 = parseFloat(formData.hasil_4x4);
    const hasilReject = parseFloat(formData.hasil_reject);
    
    if (hasil4x4 + hasilReject > jumlahMasuk) {
      toast.error('Total hasil (4x4 + Reject) tidak boleh melebihi jumlah masuk');
      return;
    }
