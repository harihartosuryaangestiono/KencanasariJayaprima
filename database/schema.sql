-- =================================================================
-- SKEMA DATABASE UNTUK PLYWOOD MONITORING SYSTEM (VERSI PERBAIKAN)
-- =================================================================

-- Bagian 1: Menghapus semua tabel yang ada (DROP TABLES)
-- Urutan penghapusan dibalik dari urutan pembuatan untuk menghindari error foreign key.
-- Tabel yang paling banyak menjadi referensi (seperti 'users') dihapus paling akhir.
-- Penggunaan 'CASCADE' akan otomatis menghapus semua constraint yang bergantung pada tabel ini.
...
-- =================================================================
-- Bagian 2: Membuat struktur tabel (CREATE TABLES)
-- Tabel dibuat berdasarkan dependensi, tabel master dibuat terlebih dahulu.
-- =================================================================
...
-- Tabel Log Produksi: Pressdryer
CREATE TABLE pressdryer_log (
    id SERIAL PRIMARY KEY,
    mesin_id INTEGER REFERENCES mesin_pressdryer(id),
    pallet_id INTEGER REFERENCES transaksi_bahan(id), -- Mengasumsikan pallet merujuk ke satu batch transaksi bahan
...
-- Tabel Log Produksi: Core Builder
...
-- Tabel Log Produksi: Scraff Join
...
CREATE TABLE setting_plywood (
    ...
    -- Kolom hasil OK/Reject lebih cocok di log Hotpress, bukan di setting
    keterangan TEXT,
    user_id INTEGER REFERENCES users(id),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- Tabel Log Produksi: Hotpress
...
-- Tabel Inventory: Finished Goods
...
-- =================================================================
-- Bagian 3: Memasukkan data awal (INSERT INITIAL DATA)
-- =================================================================
...
-- Insert Data Master (dengan ketergantungan ke tabel 'users')
...
-- Insert Data Transaksi (dengan ketergantungan ke 'suppliers', 'gudang', 'users')
...
-- =================================================================
-- Bagian 4: Membuat Indexes untuk Performa Query
-- =================================================================
...
CREATE INDEX idx_setting_plywood_tipe ON setting_plywood(tipe_plywood);
CREATE INDEX idx_finished_goods_tipe ON finished_goods(tipe_plywood);
