...
router.post('/setting-plywood', authenticateToken, authorizeRoles('PRODUKSI'), [
  ...
  try {
    ...
    // Insert setting plywood (tanpa hasil_ok dan hasil_reject karena tidak ada di schema)
    const result = await client.query(
      `INSERT INTO setting_plywood 
       (tipe_plywood, shortcore_qty, longcore_qty, face_qty, back_qty, lem_qty, keterangan, user_id)
    ...
    res.status(500).json({ 
      success: false,
      error: 'Server error',
      message: error.message || 'Terjadi kesalahan saat menyimpan setting plywood'
    });
  } finally {
    client.release();
  }
...
router.get('/logs/:type', authenticateToken, authorizeRoles('PRODUKSI', 'BOS'), async (req, res) => {
  try {
    ...
    if (start_date) {
      query += ` AND ${type === 'hotpress' ? 'hl' : type === 'repair' ? 'rl' : type === 'core-builder' ? 'cbl' : 'sjl'}.created_at::date >= $${paramCount}`;
